...
def create_capacity_tear_sheet(returns, positions, transactions,
    ...
    """
    txn_daily = capacity.daily_txns_with_bar_data(transactions, market_data)

    days_to_liquidate = capacity.days_to_liquidate_positions(
        positions,
        market_data,
        max_bar_consumption=liquidation_daily_vol_limit)

    print("Max days to liquidation for each traded name, "
          "assuming a 20% limit on daily bar consumption \n"
          "and trailing 5 day mean daily volume. \n"
    ...
    utils.print_table(
        max_days_by_ticker[max_days_by_ticker.days_to_liquidate > 1])

    last_n_days_dtl = days_to_liquidate.loc[
        days_to_liquidate.index.max() - pd.Timedelta(days=last_n_days):]

    max_days_by_ticker = capacity.get_max_days_to_liquidate_by_ticker(
        positions, last_n_days_dtl)

    print("Last {} trading days:".format(last_n_days))
    utils.print_table(
        max_days_by_ticker[max_days_by_ticker.days_to_liquidate > 1])

    llt = capacity.get_low_liquidity_transactions(txn_daily, market_data)
    print(
        'Tickers with daily transactions consuming >5% of daily bar (all backtest):')
    utils.print_table(
        llt[llt['max_pct_bar_consumed'] > trade_daily_vol_limit] * 100)

    # Can't slice datetime axis with dupes. Workaround:
    tr = txn_daily.assign(td=txn_daily.index.values)
    recent_txn_daily = tr[
        tr.td > tr.td.max() - pd.Timedelta(days=last_n_days)].drop('td', axis=1)

    llt = capacity.get_low_liquidity_transactions(
        recent_txn_daily, market_data)
    print("Tickers with daily transactions consuming >5% of daily bar "
          "(last {} trading days):".format(last_n_days))
    ...
    plotting.plot_capacity_sweep(returns, txn_daily, bt_starting_capital,
                                 min_pv=100000,
                                 max_pv=300000000,
                                 step_size=1000000,
